...
本机制上线后，`level-01.md`~`level-09.md` 已全部按符文朝向规则重新设计，并通过
RCMap 1.1 明确记录起始符文方向、标准解和 `rune.DOWN` 终态。现有 9 个 Unity 关卡场景
（`Chapter1_Scene1`~`Chapter1_Scene9`）尚未按这些 RCMap 重建；在场景通过对应 GDD
验收前，应将 RCMap 视为预期布局，将 Unity 场景视为未完成实现。
...
| `level-01.md`~`level-09.md` 与现有 9 个 Unity 关卡场景 | 关卡 GDD 已按符文朝向和 RCMap 1.1 完成重新设计；Unity 场景仍待按对应 RCMap 重建和验收，不能以当前场景反向覆盖设计文档 |
