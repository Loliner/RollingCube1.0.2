...
`level-01.md`~`level-09.md` 已按符文朝向规则设计，并通过 RCMap 1.1 记录起始符文方向、
标准解和 `rune.DOWN` 终态。9 个 Unity 关卡场景已按这些 RCMap 搭建，主路线由
`Chapter1CompletionTests` 覆盖；RCMap 仍是关卡空间结构的权威来源。
...
| GDD、场景与生成器不一致 | RCMap 是设计来源；修改布局时同步更新对应 GDD 与 `Chapter1SceneBuilder`，并用主路线测试复核 |
